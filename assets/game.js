...
const STAGES=[
 {zone:'عالم الحروف',zoneEn:'Letter World',place:'قلعة البحرين',skill:'الحروف والحركات',skillEn:'Letters & short vowels',key:'letters',icon:'<span class="ab-icon" aria-hidden="true">أب</span>',reward:'🔑',rewardName:'مفتاح الحروف',canDo:'أستطيع أن أميّز الحروف والحركات الأساسية.',canDoEn:'I can recognise letters and short vowels.',desc:'استمع إلى الصوت ثم اختر الحرف.',descEn:'Listen to the sound, then choose the letter.'},
...
const SKILL_NAMES={letters:'الحروف والحركات',sounds:'تمييز الأصوات',pronunciation:'النطق',wordReading:'قراءة الكلمات',wordMeaning:'فهم المفردات',sentenceBuild:'بناء الجملة',sentenceListen:'الاستماع للجمل',sentenceOrder:'ترتيب الجملة',fluency:'القراءة والفهم',writing:'الكتابة'};
...
/* ================= 1) الحروف والحركات ================= */
function lettersItems(t,n,ctx){let pool=BANK.letters[t];if(ctx.remedial)pool=pool.filter(x=>!x.q);const src=boostWeak(freshSample('letters'+t,pool,n,!ctx.remedial),pool);
 ...
 return items}
function actLetters(ctx){const t=ctx.remedial?0:tierFor('letters');runMCQ('letters',lettersItems(t,ctx.remedial?3:6,ctx),ctx,'الحروف والحركات','Letters & short vowels')}
...
function showLetter(i){const L=ALPHABET[i];document.querySelectorAll('.alpha-btn').forEach(b=>b.classList.toggle('sel',+b.dataset.i===i));const vowels=L.l==='ا'?[['أَ','a'],['إِ','i'],['أُ','u']]:[[L.l+'َ','fatḥa'],[L.l+'ِ','kasra'],[L.l+'ُ','ḍamma']];const long=L.l==='ا'?'':`${L.l}َا`;
 ...
 <h4>الحركات ${G('Short vowels')}</h4><div class="lc-row">${vowels.map(([v,n])=>`<button class="lc-chip" data-say="${esc(v)}"><b>${v}</b><small dir="ltr">${esc(translit(v)||n)}</small></button>`).join('')}${long?`<button class="lc-chip" data-say="${esc(long)}"><b>${long}</b><small dir="ltr">${esc(translit(long))}</small></button>`:''}</div>
